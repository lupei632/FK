/******************************************************************
 ...
 Revision History:
   ...
   04/29/2000	Lupei Zhu	documentation.
   ...
   09/08/2021	Lupei Zhu	delete the -T option and set -A to be optional.
				delete the -P option by making -G optional.
				add zeta, chi and potency tensor/VTI inputs.
   02/25/2022	Lupei Zhu	handle explosion source only (nn=1).
   07/20/2022	Lupei Zhu	use trapezoid trap() in cap/.
   07/11/2024	Lupei Zhu	change VTI to TTI.
******************************************************************/
#include <stdio.h> 
#include <math.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
  int	i,j,k,nn,ns,npt,error,intg,diff,inpType,filter;
  char	nam[128],outnm[128],*ccc,com[3]={'z','r','t'};
  float	coef,rad[6][3],m0,az,*grn,*syn[3],*src,*pt,disp[3],mt[3][3],tti[7],stiff[6][6];
  ...
  for (i=1; !error && i < argc; i++) {
	...
	switch(argv[i][1]) {
	   ...
	   case 'J':
 	   ...
 	   case 'M':
	      inpType = sscanf(&argv[i][2], "%f/%f/%f/%f/%f/%f/%f",&m0,&mt[0][0],&mt[0][1],&mt[0][2],&mt[1][1],&mt[1][2],&mt[2][2]);
	      break;
	   ...
	   case 'Q':
 	   ...
 	   case 'V':
	      j = sscanf(&argv[i][2], "%f/%f/%f/%f/%f/%f/%f",tti,tti+1,tti+2,tti+3,tti+4,tti+5,tti+6);
	      if (j<7) tti[6]=0.; if (j<6) tti[5]=0;
	      if (j<5) tti[4]=0.; if (j<4) tti[3]=0.; if (j<3) tti[2]=0.;
	      if (j<2) tti[1]=1.732;
	      if (j<1) tti[0]=1.;
	      ThomsenTTI(tti,stiff);
	      break;
  ...
  if(argc < 3 || error ) {
     fprintf(stderr,"Usage: %s -Mmag([[/Strike/Dip]/Rake[/ISO[/CLVD]]]|/Mxx/Mxy/Mxz/Myy/Myz/Mzz) -OoutName.z [-Aazimuth] ([-SsrcFunctionName] | -Ddura[/rise]]) [-Ff1/f2[/n]] ([-I] | [-J]) [-V[mu[/vpvs[/epsilon[/gamma[/delta]]]]]] [-GFirstCompOfGF]\n\
   ...
   -M Specify source magnitude and geometry or moment-tensor/potency-tensor (see -V).\n\
      For double couple, mag=Mw, strike/dip/rake are in A&R convention. Need GF.[0-8].\n\
      For explosion, only needs mag=M0 in dyne-cm (or P0) and GF.[a-c].\n\
      For single force. mag is in dyne, only strike and dip are needed. Need GF.[0-5].\n\
      For moment-tensor, mag=M0 in dyne-cm, followed by tensor components, x=N,y=E,z=Down.\n\
          or use strike/dip/rake/ISO/CLVD. Need GF .[0-8,a-c]\n\
 ...
 Optional:\n\
   ...
   -V Input strike/dip/rake/iso/clvd are potency tensor parameters of an event\n\
      in a TTI medium of muH/vpV2vsH/epsilon/gamma/delta/tilt/azimuth (1/1.732/0/0/0/0/0, off).\n\
   Examples:\n\
...
earthquake (strike 355, dip 80, rake -70), use:\n\
  ...
  if (inpType==3) {	// single force source
     nn = 2;
     sf_radiat(az-mt[0][0],mt[0][1],rad);
     m0 = m0*1.0e-15;
  } else {		// moment/potency tensor sources (including EXP and DC)
     if (inpType>1&&inpType<7) m0 = pow(10.,1.5*m0+16.1);
     m0 = m0*1.0e-20;
     nn = 4;		// in the order of DD, DS, SS, and EXP
     if (inpType==1) nn=1; 		// explosion source
     if (inpType<6) mt[1][2] = 0.;	// default CLVD parameter chi=0
     if (inpType<5) mt[1][1] = 0.;	// default ISO parameter zeta=0
     if (inpType!=7) nmtensor(mt[1][1],mt[1][2],mt[0][0],mt[0][1],mt[0][2],stiff,mt);
     fprintf(stderr,"Moment tensor %8.3e %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f Nm\n",m0*1.0e13,mt[0][0],mt[0][1],mt[0][2],mt[1][1],mt[1][2],mt[2][2]);
     if (az<0.) {
